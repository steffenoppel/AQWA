...
######################################################################################################
# 
# 1. READ IN THE DATA THAT HAVE BEEN FORMATTED FOR WINBUGS
# 
#######################################################################################################


load("C:\\STEFFEN\\RSPB\\AquaticWarbler\\Analysis\\Trend_monitoring\\AQWA_BMM_input.RData")
load("S:\\ConSci\\DptShare\\steffenoppel\\RSPB\\AquaticWarbler\\Analysis\\Trend_monitoring\\AQWA_BMM_input.RData")
...
######################################################################################################
# 
# MODEL 1: Simple Binomial-mixture model for trend estimation as described by Kéry et al. 2009 - no covariates
# 
######################################################################################################
...
T = ncol(y)
nyears = dim(y)[3]
win.data <- list(M = y, nsite = R, nrep = T, nyear=nyears, primocc=seq(1:nyears))
...
params <- c("loglam","lam.site","totalN","N", "fit", "fit.new","trend")
...
# Call WinBUGS from R (BRT 8800 seconds for 400000 iterations)
#model1 <- bugs(win.data, inits, params, "NmixTrend.txt", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, debug = TRUE, bugs.directory = bugs.dir, working.directory = getwd())
model1 <- jags(win.data, inits, params, "NmixTrend.jags", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb)
model1<-model1$BUGSoutput
