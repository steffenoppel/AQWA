...
sink("models/AQWA.IPM.Jaume.Randeff.jags")
cat("
model {
...
#--------------------------------------------
   ...
   for (t in 2:ncountyears){
        ...
        Nfemdb[t-1]  ~ dbin(db[t-1],round((Ntot[t-1])*(1-prop.males)))  ## random draw of double brooding females
      	chicks[t-1] <- (Ntot[t-1])*(1-prop.males)* fec1[t-1] + Nfemdb[t-1]*fec2[t-1]			# total fecundity
...
parameters <- c("Ntot","mphij","mphia","mfec1","mfec2","mean.lambda","prop.males", "phij", "phia", "fec1", "fec2")
...
#First result: confidence intervals in population projections are narrower, although larger than in the model with fixed productivity values
par(mfrow = c(1,1))
plot(ipm.model$mean$Ntot, type = "l", lwd = 2, col = "blue", ylim = c(0,650), ylab = "Counts", xlab = "Year", main = "Population counts")
...
write.table(out, "output/AQWA_GER_model_JAUME_randeff_output.csv", sep=",")
