...
#Add time into the age matrix
aget.mat <- age.mat
aget.mat[,2:5] <- age.mat[,2:5] + 1

#Code 1: Juvenile survival at t = 1
#Code 2: Juvenile survival at t >= 2
#Code 3: Adult survival at t >= 2 (adult survival cannot be estimated for t = 1, as all birds were released as fledglings)
...
sink("models/AQWA.IPM.surv.Scenarios.vb.jags")
cat("
model {
...
for(a in 1:3){
  ...
  mphi[a] ~ dbeta(1.2,1.2) #Same priors for every estimate
  ### survival of adult birds (code 3) for estimation - NOT USED IN POPULATION PROCESS BECAUSE BASED ON TOO FEW DATA
...
for (t in 2:(ncountyears+nprojyears)){

   phij[t] <- mphi[2]			# fixed effect for every occasion - temporal variability, survival of juveniles at t = 1 is different
   fec1[t] <-mfec1
}
...
inits <- function(){list(
  ...
  mphi= c(runif(1, 0.15,0.25), runif(1, 0.25,0.35), runif(1, 0.45,0.55)),    ## using rbeta results in crazy output and model does not converge at all
...
ipm.model <- jags(jags.data,
                  inits,
                  parameters,
                  "models/AQWA.IPM.surv.Scenarios.vb.jags",
...
write.table(out, "output/AQWA_GER_model_nscenarios_output.vb.csv", sep=",")
...
#Extract key metrics: is the population declining or increasing at the end of the projections, compared to the end of the releases?

#This is a calculation on the averages. But it is way more informative to do it on the posteriors

ntotdf <- ntotdf %>% 
  mutate(relend = 0)

ntotdf$relend[c(27, 79, 132, 168, 220, 272, 309, 361, 413, 450, 502, 554, 591, 643, 695, 732, 784, 836, 873, 915)] <- 1

tmp <- c(27, 79, 132, 168, 220, 272, 309, 361, 413, 450, 502, 554, 591, 643, 695, 732, 784, 836, 873, 915)
tmp2 <- tmp + 940

ntotdf$relend[tmp2] <- 1

Ncompar <- ntotdf %>%
  filter(year == 47 | relend == 1)

res <- Ncompar %>% 
  group_by(Scenario) %>%
  summarise(Difference = Ntot[relend == 0] - Ntot[relend == 1])

#On the projections:

dim(ipm.model$sims.list$Ntot)
...
#Calculations accounting for uncertainty
