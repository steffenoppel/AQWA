...
sink("models/AQWA.IPM.surv.Scenarios.survconstant.jags")
cat("
model {
...
for (t in 1:(ncountyears+nprojyears)){
   ...
   phij[t] <- mphi			# fixed effect for every occasion - no temporal variability
   phia[t] <- mphi			# fixed effect for every occasion - no temporal variability
   fec1[t] <-mfec1
}
...
inits <- function(){list(
  ...
  mphi= .35,    ## using rbeta results in crazy output and model does not converge at all
  mfec1 = runif(1, 2.8,3.8),
  mfec2 = runif(1, 1.9,3.0))}

# Parameters monitored
parameters <- c("mean.p","mphi","mfec1","mfec2","mean.lambda","prop.males","db", "Nfemdb",
                "proj.lambda","Ntot")
...
ipm.model <- jags(jags.data,
                  inits,
                  parameters,
                  "models/AQWA.IPM.surv.Scenarios.survconstant.jags",
...
write.table(out, "output/AQWA_GER_model_nscenarios_output.survconstant.csv", sep=",")
...
poptrendspast
ggsave("output/Past_population_trajectories_survconstant.jpg", width=241,height=191, quality=100, units="mm")
...
ggsave("output/Extinction_probability_survconstant.jpg", width=351,height=191, quality=100, units="mm")
...
ggsave("output/Future_pop_growth_rates_survconstant.jpg", width=235,height=181, quality=100, units="mm")
