...
sink("models/AQWA.IPM.surv.Scenarios.survsex.jags")
cat("
model {
...
# SURVIVAL PRIORS FOR SEX GROUPS

#Sex groups only

for(a in 1:2){
  mphi[a] ~ dbeta(1.2,1.2) 
}
      for (i in 1:n.marked){
...
for (t in 1:(ncountyears+nprojyears)){
   ...
   phij[t] <- mphi[2]			# fixed effect for every occasion - no temporal variability
...
#--------------------------------------------
   ...
   for (t in 2:ncountyears){
		...
		NadSurv[1,t] ~ dbin(mphi[2],round((Ntot[1,t-1])))
	}
      ...
      for (t in (ncountyears+1):(ncountyears+nprojyears)){
            ...
            Nfem.breed2[ns,t-ncountyears] ~ dbin(db[ns,t-ncountyears],round(Nfem.breed1[ns,t-ncountyears]))  
      	    ...
      	    chicksrd[ns,t-1] <- round(chicks[ns,t-1]) + releases[ns,t-ncountyears]
		        ...
		        NadSurv[ns,t] ~ dbin(min(1,mphi[2]*impsurv[ns]),round((Ntot[ns,t-1])))
...
inits <- function(){list(
  ...
  mphi= c(.45,.35),    ## using rbeta results in crazy output and model does not converge at all
...
ipm.model <- jags(jags.data,
                  inits,
                  parameters,
                  "models/AQWA.IPM.surv.Scenarios.survsex.jags",
...
write.table(out, "output/AQWA_GER_model_nscenarios_output.survsex.csv", sep=",")
